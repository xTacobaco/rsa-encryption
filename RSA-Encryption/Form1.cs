...
namespace RSA_Encryption {
    public partial class Form1 : Form {
        ...
        private void btnGenerateClick(object sender, EventArgs e) {
            ...
            long cp = EuclideanAlgorithm.Coprime(m);
            txtLog.Text += $"( + ) e = {cp}\n";
            long d = DiophantineSolver.MultiplicativeInverse(m, cp).Item2;
            txtLog.Text += $"( + ) d = {d}\n";
        ...
        private void btnEncrypt_Click(object sender, EventArgs e) {
            ...
            txtOutput.Text = Encryption.Encrypt(publicKey, int.Parse(txtInput.Text));
        }

        private void btnDecrypt_Click(object sender, EventArgs e) {
            txtLog.Text += $"( + ) Decrypting {txtInput.Text} using the private key." + "\n";
            txtOutput.Text = Encryption.Decrypt(privateKey, int.Parse(txtInput.Text));
        }
    }
}
